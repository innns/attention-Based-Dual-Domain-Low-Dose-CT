...
class TransformerModel(tf.keras.Model):
    def __init__(self, transformer_layers,num_features ,sparse_scale= 2,num_heads = 8 ,expend_units=4):
        ...
        self.sinModule.append(
            tf.keras.layers.Conv2D(sparse_scale-1 , kernel_size=1, padding='same', name='sine_conv4'))

        for i in range( transformer_layers):
            self.transformer_block.append(TransformerBlock(num_features=self.num_features ,num_heads=num_heads,mlp_dim=self.num_features * expend_units,dropout=0.1,name="mytrans"+str(i)))
    ...
    def call(self, train_batch, training=False):  # 定义正向传播过程
        ...
        sin_interp_t = self.concatSino(sin_interp)
        T_sin_in = tf.squeeze(self.ShuffleSino(sin_interp_t, sin_in), 3)
        ...
        cnn_out = self.ShuffleSino(x,sin_in)

        for i in range(self.transformer_layers):
            T_sin_in = self.transformer_block[i](T_sin_in)
        T_out = tf.expand_dims(T_sin_in,3)
        fusion_sin_out = cnn_out+T_out

        fbp_cnn_out = self.fbpModule(cnn_out[:, :, 0:self.num_features - 3, :])
        fbp_T_out = self.fbpModule(T_out[:,:,0:self.num_features-3,:])
        fbp_fusion_out = self.fbpModule(T_out[:, :, 0:self.num_features - 3, :])
        model_out = [cnn_out , T_out, fusion_sin_out, fbp_cnn_out,fbp_T_out, fbp_fusion_out,T_sin_in]
        if training:
    ...
    def setTrainMode(self, sinLayers=True,fbpLayer=False, ctLayers=False):
        ...
        self.trainable_var = []
        if sinLayers:
            self.trainable_var.extend(self.trainable_variables[0:len(self.trainable_variables)-3])
        if fbpLayer:
            self.trainable_var.extend(self.trainable_variables[len(self.trainable_variables)-3:len(self.trainable_variables)])
    ...
    def loss(self,sin_label, ct_label,model_out, weights=(1.0, 1.0, 1.0,1.0)):
        ...
        if self.trainMode[0]:  # sine loss
            loss = weights[0] * tf.reduce_mean(tf.math.square(model_out[2] - sin_label))
