...
def save_data(data_batch, test_out, i, c):
    sin_out, fbp_out, ct_out, sin_interp = test_out
    ...
    cv2.imwrite('./Result/our/' + str(c) + 'x/our_seq/seq_' + str(i) +'_'+str(c)+'x'+ '_fbp_out.bmp', fbp_out)
...
def test_mymodel(ckpt='./256x256/weights/new_model_lambda=0.5',testDataDir = '',test_sz =2 ,c=2):
    ct_model =  dual_seq_model(2, 360, c)
...
if __name__ == '__main__':
    ...
    i = 0
    iterNo = 300
    ckpt = './256x256_seq/' + str(c) + 'x_weights_our_' + str(i) + '_epoch' + str(iterNo) + 'iter' + '/ckpt/'
    test_sz = 2
