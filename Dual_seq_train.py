...
class dual_seq_model(tf.keras.Model):
    def __init__(self, transformer_layers,num_features ,sparse_scale= 2,ckpt_sine = None):
        ...
        # self.Fusion_model = Funsion_model(2)
    ...
    def call(self,train_batch, training=False):
        if training:
            ...
            return [sin_out ,fbp_out,ct_out,sin_interp]
    ...
    def setTrainMode(self, sinLayers=True,fbpLayer=False, ctLayers=False):
        ...
        self.trainable_var = []
        if sinLayers:
            self.trainable_var.extend(self.trainable_variables[0:len(self.trainable_variables)-31])
        if ctLayers:
            self.trainable_var.extend(self.trainable_variables[len(self.trainable_variables)-28:len(self.trainable_variables)])
...
def compute_psnr(data_batch, model_out):
    ...
    return [  psnr(sin_label_large, sin_interp_large),psnr(sin_label_large, sin_out_large),psnr(ct_label, fbp_out),psnr(ct_label, ct_out)]
...
def train(epoch=3, batch_sz=1, c=2,ckpt_sine=None):
    ...
    ct_model = dual_seq_model(2,360,c,ckpt_sine)
        ...
        if i <= 80:
            ...
            for iterNo, data_batch in enumerate(train_data.batch(batch_sz)):
                ...
                if iterNo % 100 == 0:
                    ...
                    ckpt = './256x256_seq/' + str(c) + 'x_weights_our_' + str(i) + '_epoch' + str(
                        iterNo) + 'iter' + '/ckpt/'
                    ct_model.save_weights(ckpt)
                    loss.append(loss_epoch)
                    loss_epoch = 0
        if 80<= i <= 100:
            ct_model.setTrainMode(True, False, True)
            loss_epoch = 0
            for iterNo, data_batch in enumerate(train_data.batch(batch_sz)):
                ...
                if iterNo % 100 == 0:
                    ...
                    # show_image(val_data, val_out)
                    print("epoch: ", i, ":", "psnr_valid", psnr_valid)
                    # ckpt = './256x256_seq/' + str(c) + 'x_weights_our_' + str(i) + '_epoch/ckpt/'
                    ckpt = './256x256_seq/' + str(c) + 'x_weights_our_' + str(i) + '_epoch' + str(
                        iterNo) + 'iter' + '/ckpt/'
                    ct_model.save_weights(ckpt)
                    loss.append(loss_epoch)
...
if __name__ == '__main__':
    ckpt_sine = r'E:\A.Study\Uestc\毕业设计\模型参数\参数\正弦域单独实验参数\our\2_0x_weights_4_epoch\ckpt/'
    train(epoch=100, batch_sz = 2, c=2,ckpt_sine=ckpt_sine)
